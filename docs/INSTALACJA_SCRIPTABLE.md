...
Ta instrukcja pokazuje, jak zainstalować widgety Shoper z repozytorium `scriptable-shoper` w aplikacji Scriptable na iPhonie lub iPadzie.
...
- [3. Wymagane uprawnienia API](#3-wymagane-uprawnienia-api)
...
Nie wrzucaj tych danych bezpośrednio do głównego skryptu widgetu.

Dane API najlepiej przechowywać w Keychain aplikacji Scriptable. Dzięki temu token nie trafia do pliku JS i nie zostanie przypadkowo opublikowany na GitHubie.
...
## 4. Zapisanie Client ID i Token API w Keychain Scriptable
...
Po poprawnym zapisaniu danych możesz zostawić skrypt setupowy w Scriptable albo go usunąć. Same dane zostaną zapisane w Keychain.

---

## 5. Przygotowanie głównego skryptu widgetu
...
## 6. Konfiguracja widgetu

Na początku skryptu ustaw najważniejsze opcje.

### Konfiguracja `shoper_1.js`
...
Czas odświeżania widgetu ustawisz zmienną:
...
Tryb demo służy do testowania wyglądu widgetu bez łączenia się z API Shopera.
...
## 8. Test działania w Scriptable

Po wklejeniu i skonfigurowaniu kodu uruchom skrypt w Scriptable przyciskiem ▶️.
...
## 9. Dodanie widgetu na ekran iPhone’a
